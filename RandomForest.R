...
df_TCD$SeniorYN <- 'Yes'
df_TCD$SeniorYN[df_TCD_thin$SeniorCitizen == 0] <-'No' 
...
#df_TCD  <- df_TCD_thin[-c(3,7)]  #needed to run only 1 time
str(df_TCD)
...
churn.forest1 <- randomForest(formula = Churn ~ .,data = df_TCD, ntree = 500, subset=training.set,importance = TRUE)
